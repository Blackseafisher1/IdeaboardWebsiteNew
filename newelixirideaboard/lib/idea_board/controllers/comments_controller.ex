  ...
  defp ws_dispatch(conn) do
    ...
    case action do
      "list" ->
        ...
        html = IdeaBoard.Renderer.render_raw("ideas/_comments_section", %{comments: comments, idea_id: idea_id, user: user}, conn)
      ...
      "create" ->
        case IdeaBoard.IdeasCommentsService.create(user, idea_id, text) do
          {:ok, comment} ->
            ...
            html = IdeaBoard.Renderer.render_raw("ideas/_comment_item", %{comment: comment, user: user}, conn)
            assign(conn, :rendered_html, html)
