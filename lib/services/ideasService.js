...
async function createIdea(userId, data, file) {
  ...
  try {
    ...
    const ideasFilesService = require('./ideasFilesService'); // Lazy-Load, um zirkuläre Abhängigkeiten zu vermeiden
...
async function addIdeaTags(ideaId, userId, tagsInput) {
  ...
  try {
    ...
    // 1 Punkt für das Hinzufügen von Tags vergeben (Engagement)
...
async function updateIdeaStatus(ideaId, status, userId, userRole) {
  ...
  // Aktuellen Status laden, um Regeln durchzusetzen (verhindert manuelles Setzen auf 'umgesetzt'
  // und verhindert Änderungen, sobald der Status bereits 'umgesetzt' ist)
...
async function fetchIdeas(userId, query = {}) {
  ...
  if (isSearch) {
    ...
    // SQL-Fulltext-Suche für Titel/Autor und Beschreibung verwenden
