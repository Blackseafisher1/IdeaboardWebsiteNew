...
async function getMetrics() {
    const newIdeasRows = await db.query(`-- sql
    ...
    const activeUsersRows = await db.query(`-- sql
    ...
    const activeCategoriesRows = await db.query(`-- sql
...
async function getTopIdeas(limit = 3) {
    const topIdeas = await db.query(`-- sql
...
async function getActiveUsers(limit = 10) {
    const activeUsers = await db.query(`-- sql
...
async function getPopularCategories(limit = 5) {
    const popularCategories = await db.query(`-- sql
...
async function getProjectStats() {
    const projectStats = await db.query(`-- sql
...
async function getMonthlyStats(months = 6) {
    const monthlyStats = await db.query(`-- sql 
...
async function getTopUsers(limit = 5) {
    const topUsers = await db.query(`-- sql
...
async function getIdeaCardData(ideaId) {
    const ideaRows = await db.query(`-- sql
      SELECT 
        i.idea_id,
        i.title,
        i.description,
