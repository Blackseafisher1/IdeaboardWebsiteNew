...
async function fetchSurveys({ userId, userRole, type = 'all', search = '', limit = 10, offset = 0 }) {
    ...
    if (search && search.trim()) {
        baseQuery += ' AND (bounded_edit_dist(LOWER(s.title), LOWER(?), 4) <= 4 OR bounded_edit_dist(LOWER(COALESCE(s.description, ?)), LOWER(?), 6) <= 6 OR bounded_edit_dist(LOWER(u.username), LOWER(?), 3) <= 3)';
        baseParams.push(search.trim(), '', search.trim(), search.trim());
    }
