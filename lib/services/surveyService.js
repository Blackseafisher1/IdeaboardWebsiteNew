...
async function fetchSurveys({ userId, userRole, type = 'all', search = '', limit = 10, offset = 0 }) {
    ...
    if (search && search.trim()) {
        const q = `%${search.trim()}%`;
        baseQuery += ' AND (LOWER(s.title) LIKE ? OR (s.description IS NOT NULL AND s.description != \'\' AND LOWER(s.description) LIKE ?) OR (u.username IS NOT NULL AND LOWER(u.username) LIKE ?))';
