...
async function fetchSurveys({ userId, userRole, type = 'all', search = '', limit = 10, offset = 0 }) {
    ...
    if (search && search.trim()) {
        const q = search.trim();
        baseQuery += ' AND (LOWER(s.title) LIKE CONCAT(\'%\', LOWER(?), \'%\') OR LOWER(COALESCE(s.description, \'\')) LIKE CONCAT(\'%\', LOWER(?), \'%\') OR LOWER(u.username) LIKE CONCAT(\'%\', LOWER(?), \'%\') OR bounded_edit_dist(LOWER(s.title), LOWER(?), 5) <= 5 OR bounded_edit_dist(LOWER(COALESCE(s.description, \'\')), LOWER(?), 6) <= 6 OR bounded_edit_dist(LOWER(u.username), LOWER(?), 4) <= 4)';
