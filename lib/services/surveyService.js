...
async function fetchSurveys({ userId, userRole, type = 'all', search = '', limit = 10, offset = 0 }) {
    ...
    if (search && search.trim()) {
        const fuse = new Fuse(allSurveys, {
            keys: ['title', 'description', 'creator_name'],
            threshold: 0.3
        });
        // Führe Fuzzy-Suche mit Fuse.js aus und extrahiere die Items
        filteredSurveys = fuse.search(search).map(r => r.item);
    }
