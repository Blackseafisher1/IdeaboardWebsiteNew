...
async function fetchSurveys({ userId, userRole, type = 'all', search = '', limit = 10, offset = 0 }) {
    ...
    if (search && search.trim()) {
        const q = search.trim();
        baseQuery += ' AND (bounded_edit_dist(LOWER(s.title), LOWER(?), 6) <= 6 OR (s.description IS NOT NULL AND s.description != \'\' AND bounded_edit_dist(LOWER(s.description), LOWER(?), 8) <= 8) OR (u.username IS NOT NULL AND bounded_edit_dist(LOWER(u.username), LOWER(?), 4) <= 4))';
