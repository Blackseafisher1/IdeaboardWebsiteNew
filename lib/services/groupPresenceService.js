...
const groupWaiters = new Map(); // groupId -> Set von Waiter-Objekten
/** @type {Map<number, Map<number, number>>} */
const groupPresence = new Map(); // groupId -> Map(userId -> Verbindungsanzahl)
...
function localNotifyGroupWaiters(groupId, payload = null, options = null) {
    ...
    if (!waiters) return;
    for (const waiter of Array.from(waiters)) {
        // Wenn options.excludeUserId gesetzt ist, diesen Nutzer nicht benachrichtigen
