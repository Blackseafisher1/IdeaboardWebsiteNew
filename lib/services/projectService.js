        ...
        if (searchQuery.trim()) {
            const fuse = new Fuse(allProjects, {
                keys: [
                    ...
                    { name: 'contact_person_name', weight: 0.5 }
                ],
                threshold: 0.7
            });
            // Verwende Fuse.js zur Fuzzy-Suche und extrahiere die Items
            filteredResults = fuse.search(searchQuery.trim()).map(r => r.item);
        }
    const startIndex = (page - 1) * limit;
    const pageProjects = filteredResults.slice(startIndex, startIndex + limit);
