...
function parseOptionalPositiveInt(value, fieldLabel) {
    ...
    if (!/^\d+$/.test(text)) {
        throw new Error(`BAD_REQUEST: ${fieldLabel} muss eine gültige Zahl sein`);
    }
...
function parseProgressValue(value) {
    ...
    if (!Number.isFinite(parsed)) {
        throw new Error('BAD_REQUEST: Fortschritt muss eine Zahl zwischen 0 und 100 sein');
    }
...
const ProjectService = {
    ...
    async getProjectsList(user, { page = 1, limit = 8, status = 'all', search = '', userOnly = false }) {
        ...
        const userOnlyValue = Array.isArray(userOnly) ? String(userOnly[userOnly.length - 1]) : String(userOnly);
        const isUserOnly = userOnlyValue === 'on' || userOnlyValue === 'true';
        ...
        if (searchQuery.trim()) {
            ...
            whereParts.push('(MATCH(p.name) AGAINST(? IN BOOLEAN MODE) OR MATCH(p.description) AGAINST(? IN BOOLEAN MODE) OR (u.username IS NOT NULL AND LOWER(u.username) LIKE CONCAT(\'%\', LOWER(?), \'%\')))');
            params.push(booleanQ, booleanQ, q);
        }
    ...
    async createProject(user, data, options = {}) {
        if (!this.canUserCreateProject(user)) {
            throw new Error('FORBIDDEN: Nur Admin und Projektleiter können Projekte erstellen');
        }
        ...
        if (!trimmedName) {
            throw new Error('BAD_REQUEST: Projektname darf nicht leer sein');
        }

        const finalContactPersonId = parseOptionalPositiveInt(contact_person_id, 'Ansprechpartner');

        // If the creator is a Projektleiter, force the contact person to themselves
        let effectiveContactPersonId = finalContactPersonId;
        if (isProjectLead(user)) {
            effectiveContactPersonId = Number.parseInt(user.id, 10);
        }

        const finalConvertIdeaId = parseOptionalPositiveInt(convertIdeaId, 'Idea-ID');

        const expectedAuthorId = parseOptionalPositiveInt(expected_author_id, 'Erwartete Autor-ID');
        ...
        try {
            ...
            if (finalConvertIdeaId) {
                ...
                const ideaStatus = String(convertIdea.status || '').toLowerCase();
                if (ideaStatus === 'umgesetzt') {
                    throw new Error('BAD_REQUEST: Diese Idee wurde bereits in ein Projekt umgewandelt');
                }
                if (ideaStatus !== 'akzeptiert') {
                    throw new Error('BAD_REQUEST: Nur akzeptierte Ideen können in Projekte umgewandelt werden');
                }
            }

            if (effectiveContactPersonId) {
                const contactRows = await conn.query(`-- sql
                    SELECT r.name as role, u.user_id
                    FROM users u
                    JOIN roles r ON u.role_id = r.role_id
                    WHERE u.user_id = ?
                `, [effectiveContactPersonId]);
                ...
                if (!isProjectLead(contactUser) && !hasAdminRole(contactUser)) {
                    ...
                    await adminService.promoteToRole(effectiveContactPersonId, 'Projektleiter', user.id, conn);
                }
            }
            ...
            try {
                ...
                const groupOwnerId = effectiveContactPersonId || user.id;
                const groupId = await groupService.createGroup({
                    name: `Projekt: ${trimmedName}`,
                ...
                if (effectiveContactPersonId && effectiveContactPersonId !== groupOwnerId) {
                    await groupService.addMember(groupId, effectiveContactPersonId, 'admin', conn);
                }
            } catch (groupErr) {
            ...
            if (effectiveContactPersonId) {
                await pointsService.addPendingDelta({ userId: effectiveContactPersonId, delta: 10, reason: 'project-created', source: 'projectService', conn });
                // Ansprechpartner automatisch zum Team hinzufügen
                await this._ensureContactInTeam(projectId, effectiveContactPersonId, conn);
            }
    ...
    async updateProject(user, id, data) {
        ...
        try {
            ...
            if (hasAdminRole(user) && contact_person_id && String(contact_person_id).trim() !== '') {
                            const parsedContactPersonId = parseOptionalPositiveInt(contact_person_id, 'Ansprechpartner');
                const contactRows = await conn.query(`-- sql
                    SELECT r.name as role, u.user_id
                    FROM users u 
                    JOIN roles r ON u.role_id = r.role_id 
                    WHERE u.user_id = ?
                            `, [parsedContactPersonId]);
                const contactUser = contactRows[0];
                ...
                finalContactPersonId = parsedContactPersonId;
            }
