...
function parseTagsInput(tagsRaw) {
  return String(tagsRaw || '')
    .split(',')
    // Normalisieren: trimmen + kleinschreiben
...
function buildBooleanQuery(rawQ) {
  ...
  for (let t of tokens) {
    t = t.trim();
    if (!t) continue;
    // FULLTEXT-Sonderzeichen escapen, um Syntaxfehler zu vermeiden
    t = t.replace(/[+\-*<>()~@"]/g, ' ');
...
/**
 * Führt die Fulltext-Suche aus (Late-Row-Lookup).
 * Bei < 3 Zeichen Fallback auf LIKE (title/description).
 * @async
 * @param {string} q - Rohsuchtext.
 * @param {string} booleanQuery - Vorbereitete boolean-mode Query.
 * @param {string} whereSql - Optionales WHERE-Fragment.
 * @param {Array} baseParams - Parameter für `whereSql`.
 * @param {number} limit - LIMIT.
 * @param {number} offset - OFFSET.
 * @param {boolean|string} [descriptionMode=true] - Beschreibungssuche.
 * @param {boolean} [debug=false] - Debug.
 * @param {string} [sortKey=''] - Sortierschlüssel.
...
async function executeFulltextSearch(q, booleanQuery, whereSql, baseParams, limit, offset, descriptionMode = true, debug = false, sortKey = '') {
  ...
  const isShort = qNorm.length < 3;

  // Description mode
  if (descriptionMode === 'desc_only' || descriptionMode === true) {
    try {
      const phraseCond = isShort
        ? `s.description LIKE CONCAT('%', ?, '%')`
        : `MATCH(s.description) AGAINST(? IN BOOLEAN MODE)`;
      const phraseParams = isShort ? [qNorm] : [booleanQuery];
      ...
      const listSql = `
        ...
        ORDER BY i.created_at DESC
      `;
      const rows = await db.query(listSql, [...(baseParams || []), ...phraseParams, Number(limit) || 0, Number(offset) || 0]);
      ...
      const countRows = await db.query(countSql, [...(baseParams || []), ...phraseParams]);
  ...
  // Default: title + author search
  try {
    let searchCond, condParams;
    if (isShort) {
      searchCond = `s.title LIKE CONCAT('%', ?, '%')`;
      condParams = [qNorm];
    } else {
      searchCond = `MATCH(s.title) AGAINST(? IN BOOLEAN MODE) OR MATCH(s.author) AGAINST(? IN BOOLEAN MODE)`;
      condParams = [booleanQuery, booleanQuery];
    }
    ...
    if (isShort) {
      const listSql = `
        ...
        ORDER BY i.created_at DESC
      `;
      rows = await db.query(listSql, [...(baseParams || []), ...condParams, Number(limit) || 0, Number(offset) || 0]);
    } else {
      const listSql = `
        SELECT i.*, u.username AS author_username
        FROM (
          SELECT s.idea_id,
            (MATCH(s.title) AGAINST(? IN BOOLEAN MODE) * 2 + MATCH(s.author) AGAINST(? IN BOOLEAN MODE)) AS search_score
          FROM ideas_search s
          JOIN ideas i ON i.idea_id = s.idea_id
          ${whereClause}
        ...
        JOIN ideas i ON i.idea_id = top.idea_id
        JOIN users u ON u.user_id = i.user_id
        ORDER BY top.search_score DESC
      `;
      rows = await db.query(listSql, [booleanQuery, booleanQuery, ...(baseParams || []), ...condParams, Number(limit) || 0, Number(offset) || 0]);
    }
    normalizeAuthor(rows);

    const countSql = `SELECT COUNT(*) AS total FROM ideas_search s JOIN ideas i ON i.idea_id = s.idea_id ${whereClause}`;
    const countRows = await db.query(countSql, [...(baseParams || []), ...condParams]);
    const total = countRows[0]?.total || 0;
    return { results: rows, total };
  } catch (e) {
    return { results: [], total: 0 };
